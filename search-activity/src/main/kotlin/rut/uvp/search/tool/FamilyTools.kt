...
class FamilyTools {
    ...
    fun findActivities(
        @ToolParam(required = false, description = "Остальные члены семьи с которыми планируется поход на мероприятие (самого пользователя указывать не нужно). Например (брат, папа, жена и т.д.)")
        members: List<String>,
        ...
        endDate: String?,
        @ToolParam(required = false, description = "Дополнительные предпочтения или интересы. Например (спорт, кино и т.д.)")
